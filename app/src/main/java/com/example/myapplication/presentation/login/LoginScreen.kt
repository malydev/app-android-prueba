    ...
    Column(Modifier.fillMaxSize().imePadding().verticalScroll(rememberScrollState()).padding(28.dp), verticalArrangement = Arrangement.spacedBy(20.dp)) {
        ...
        Card(colors = CardDefaults.cardColors(containerColor = Color.White), shape = RoundedCornerShape(24.dp)) {
            Column(Modifier.padding(22.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                ...
                OutlinedTextField(value = form.email, onValueChange = onEmailChanged, label = { Text("Correo electrónico") }, singleLine = true,
                ...
                OutlinedTextField(value = form.password, onValueChange = onPasswordChanged, label = { Text("Contraseña") }, singleLine = true,
                    ...
                    keyboardActions = KeyboardActions(onDone = { onSignIn() }), modifier = Modifier.fillMaxWidth())
                if (state is UiState.Error) Text(state.message, color = MaterialTheme.colorScheme.error)
                Button(onClick = onSignIn, enabled = !loading, modifier = Modifier.fillMaxWidth().height(54.dp), shape = RoundedCornerShape(14.dp)) {
